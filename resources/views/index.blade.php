                  ...
                  "pagination": false
                        ...
                        <div class="swiper-wrapper">
                            @foreach ($categories as $category)
                                <div class="swiper-slide text-center d-flex flex-column align-items-center">
                                    <!-- Image Section -->
                                    <a href="{{ route('shop.index', ['categories' => $category->id]) }}" class="d-block">
                                        <img loading="lazy" class="w-100 h-auto mb-2 rounded"
                                    ...
                                    <a href="{{ route('shop.index', ['categories' => $category->id]) }}"
                                        class="menu-link fw-medium text-dark text-decoration-none">
                                        {{ $category->name }}
                                    </a>
                                </div>
                            @endforeach
                                                ...
                                                <div class="product-card__price d-flex">
                                                    <span class="money price text-secondary">
                                                        @if ($sproduct->sale_price)
                                                            <s>Tk {{ $sproduct->regular_price }} </s> Tk
                                                            {{ $sproduct->sale_price }}
                                                        @else
                            ...
                            <div class="product-card product-card_style3 mb-3 mb-md-4 mb-xxl-5">
                                <div class="pc__img-wrapper">
                                    <a href="{{ route('shop.product.details', ['product_slug' => $fproduct->slug]) }}">
                                        <!-- Further reduced size for images -->
                                        <img loading="lazy" src="{{ asset('uploads/products') }}/{{ $fproduct->image }}"
                                            width="200" height="240" alt="{{ $fproduct->name }}" class="pc__img">
                                    </a>
                                </div>
                                    ...
                                    <div class="product-card__price d-flex align-items-center">
                                        <span class="money price text-secondary">
                                            @if ($fproduct->sale_price)
                                                <s>Tk {{ $fproduct->regular_price }}</s> Tk {{ $fproduct->sale_price }}
                                            @else
                                                Tk {{ $fproduct->regular_price }}
                                            @endif
                                        </span>
                                    </div>
                                </div>
